...
#' 
#' Each entry in the dataframe is also given a 'Confidence of Class'. In this
#' context, it tries to measure the sampling effort (number of OpenStreetMap
#' editors) and the time of last edit to represent uncertainty in the in the
...
#'   (default=TRUE)
#' @param amsterdam Calculate status relative to data for Amsterdam included in
...
# save(scotlandAmsterdam,file="/home/tim/github/mypackage/data/scotlandAmsterdam.RData")
# 1. name
# 2. cyclepath
# 3. road
# 4. bicycleparking
...
bicycleStatus <- function(x){
  ...
  x$'cyclepath to road ratio norm' <- round(x$'cyclepath to road ratio' / max(x$'cyclepath to road ratio'),digits=2) * 4
  
  # cycleparking  
  x$'area to bicycle parking ratio' <- round(x[,4] / x[,5],digits=6) 
  
  # normalised cycleparking  
  x$'area to bicycle parking ratio norm' <- round(x$'area to bicycle parking ratio' / max(x$'area to bicycle parking ratio'),digits=2)
  
  x$'rural weighting' <- round(round(x[,5] / x[,3],digits=4) / 3000, digits=6)
  
  # route to road ratio  
  x$'cycle route to road ratio' <- round(x[,6] / x[,3], digits=3) 
  
  # route normalised * 2 weighting  
  x$'cycle route to road ratio norm' <- round(x$'cycle route to road ratio' / max(x$'cycle route to road ratio'),digits=2) * 2
  ...
  # confidence of class needs reworking/refactoring e.g. add area size biase - fudge:  
  x$'Confidence of class' <- (x[,10] + as.numeric(substr(x[,11],1,4))) - 2010 
  x$'Confidence of class' <- (round(x$'Confidence of class' / max(x$'Confidence of class'),digits=2) * 100) + 40 #mmmmh smell the fudge
