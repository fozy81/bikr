...
bicycleTarget <- function(summary, status,completion){
...
status$'Cycle Path km Target' <- round(status$'Cycle Path km Target',digits=2)

status$'Yearly km target' <- round( status$'Cycle Path km Target' / completion,digits=2)
  
  
  x <- data.frame(cbind(status[,1], status$'Cycle Path km Target',status$'Yearly km target'))
  names(x) <- c("Name","Cycle Path km Target", "Yearly km Target")
return(x)
#  date <- Sys.time()
 # year <- format(date, "%Y")
